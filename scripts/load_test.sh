...
upload_binary_and_run() {
    local sub_id="sub-binary-$(date +%s)"

    echo "Building static Go binary..."
    CGO_ENABLED=0 GOOS=linux GOARCH=amd64 \
        go build -o /tmp/iicpc-binary-bot test-bots/contestant.go

    echo "Uploading binary bot ($sub_id)..."

    UPLOAD_RESP=$(curl -s -X POST \
        -F "submission_id=$sub_id" \
        -F "language=go" \
        -F "artifact_type=binary" \
        -F "binary_file=@/tmp/iicpc-binary-bot" \
        "http://$SANDBOX_IP:8081/upload")

    CONTAINER_IP=$(echo "$UPLOAD_RESP" | grep -o '"container_ip":"[^"]*' | cut -d'"' -f4)
    CONTAINER_PORT=$(echo "$UPLOAD_RESP" | grep -o '"container_port":[0-9]*' | cut -d':' -f2)

    if [[ -z "$CONTAINER_IP" || -z "$CONTAINER_PORT" ]]; then
        echo "❌ Failed to parse binary endpoint. Response: $UPLOAD_RESP"
        return
    fi

    ENDPOINT="${CONTAINER_IP}:${CONTAINER_PORT}"

    echo "✅ Binary Sandbox Container Ready at: $ENDPOINT"

    sleep 3

    echo "Triggering Fleet load test for binary submission..."
    ...
    echo -e "\n✅ Binary load test fired for $sub_id\n"
}
...
kubectl port-forward svc/timescaledb-service 5432:5432 >/tmp/timescaledb-pf.log 2>&1 &
PF_PID=$!
...
if [[ -n "$LEADERBOARD_IP" ]]; then
    echo "📊 Live Leaderboard UI is available at: http://$LEADERBOARD_IP"
fi